...
public class TurnSquaresIdle : MonoBehaviour {
	#region VARIABLES
	int boardWidth = 10;
	int boardHeight = 17;
	float step = 2.5f;
	
	System.Collections.Generic.List<TurnCard> cards = new System.Collections.Generic.List<TurnCard>();
	#endregion
	...
	void CreateCardMatrix()
	{
		...
		for (int i = 0; i < boardWidth; ++i)
		{
			for (int j = 0; j < boardHeight; ++j)
			{
				...
				cards.Add(go.GetComponent<TurnCard>());
	...
	void ProcessTouch(float x, float y)
	{
		...
		{
			hit.collider.GetComponent<TurnCard>().FlipCard();
		}
	}
